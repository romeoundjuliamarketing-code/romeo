-- Only surface chats that actually have at least one message.
-- Every sparring auto-creates chat settings, so without this the list showed
-- empty chats (often several with the same auto-generated title).
...
AS $$
  WITH my_sparrings AS (
    ...
    WHERE (
      ...
      -- Only chats that actually have at least one message
      AND EXISTS (
        SELECT 1 FROM sparring_group_messages m WHERE m.sparring_id = s.id
      )
  ),
  my_reads AS (
